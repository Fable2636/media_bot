...
async def check_admin(user: User) -> bool:
    ...
    Примечание:
        Эта функция используется для базовой проверки административных прав.
        Для модерации заданий требуется дополнительная проверка:
        - Суперадмины могут модерировать любые задания
        - Обычные администраторы могут модерировать только свои задания
        - Эта дополнительная проверка выполняется в обработчиках действий модерации
    """
    try:
